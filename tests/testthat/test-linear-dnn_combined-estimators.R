...
test_that("linear_dnn_combined_classification() runs successfully", {
  ...
  reg1 <- linear_dnn_combined_classification(recipe = recipe, dnn_hidden_units = c(1L, 1L), dnn_optimizer = "Adagrad")
  coefs1 <- coef(reg)
  expect_warning(predictions1 <- predict(reg))
  expect_equal(length(predictions), 32)

  # Casting "vs" to factor
  mtcars$vs <- as.factor(mtcars$vs)
  recipe <- simple_linear_dnn_combined_recipe(mtcars, response = "vs", linear.features = c("cyl"), dnn.features = c("drat"))
  reg2 <- linear_dnn_combined_classification(recipe = recipe, dnn_hidden_units = c(1L, 1L), dnn_optimizer = "Adagrad")
  coefs2 <- coef(reg)
  expect_warning(predictions2 <- predict(reg))
