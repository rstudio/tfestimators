...
hook_stop_at_step <- function(num_steps = NULL, last_step = NULL) {
  ...
  tf$python$training$basic_session_run_hooks$StopAtStepHook(
    num_steps = cast_nullable_scalar_integer(num_steps),
    last_step = cast_nullable_scalar_integer(last_step)
  )
}
                                  ...
                                  listeners = NULL)
{
  ...
  tf$python$training$basic_session_run_hooks$CheckpointSaverHook(
    ...
    save_secs = cast_nullable_scalar_integer(save_secs),
    save_steps = cast_nullable_scalar_integer(save_steps),
    saver = saver,
                              ...
                              summary_writer = NULL) 
{
  tf$python$training$basic_session_run_hooks$StepCounterHook(
    every_n_steps = cast_nullable_scalar_integer(every_n_steps),
    every_n_secs = cast_nullable_scalar_integer(every_n_secs),
