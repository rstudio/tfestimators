...
#' @param hooks List of `SessionRunHook` subclass instances. Used for callbacks
#'   inside the training loop.
#' @param steps Number of steps for which to train model. If `NULL`, train
#'   forever or train until input_fn generates the `OutOfRange` or
#'   `StopIteration` error. 'steps' works incrementally. If you call two times
#'   train(steps=10) then training occurs in total 20 steps. If `OutOfRange` or
#'   `StopIteration` error occurs in the middle, training stops before 20 steps.
#'   If you don't want to have incremental behaviour please set `max_steps`
#'   instead. If set, `max_steps` must be `NULL`.
#' @param max_steps Number of total steps for which to train model. If `NULL`,
...
#'   
#' @section Raises: ValueError: If both `steps` and `max_steps` are not `NULL`.
...
#'   raises an end-of-input exception (`OutOfRangeError` or `StopIteration`).
#' @param predict_keys list of `str`, name of the keys to predict. It is used if
#'   the `EstimatorSpec.predictions` is a `dict`. If `predict_keys` is used then
#'   rest of the predictions will be filtered from the dictionary. If `NULL`,
#'   returns all.
#' @param hooks List of `SessionRunHook` subclass instances. Used for callbacks
...
#'   dictionary of `Tensor` with labels.
#' @param steps Number of steps for which to evaluate model. If `NULL`,
#'   evaluates until `input_fn` raises an end-of-input exception.
#' @param hooks List of `SessionRunHook` subclass instances. Used for callbacks
#'   inside the evaluation call.
#' @param checkpoint_path Path of a specific checkpoint to evaluate. If `NULL`,
#'   the latest checkpoint in `model_dir` is used.
...
#'   of the global step for which this evaluation was performed.
#' @section Raises: ValueError: If `steps <= 0`. ValueError: If no model has
#'   been trained, namely `model_dir`, or the given `checkpoint_path` is empty.
#'   
#' @export
#' @family custom estimator methods
