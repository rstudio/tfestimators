...
#' @param default_value The value to use for out-of-vocabulary feature values.
#'   Default is -1.
#' @param combiner A string specifying how to reduce if the sparse column is
...
#'   The default is 1. When dimension is not None, the Tensor representing the
#'   _RealValuedColumn will have the shape of [batch_size, dimension]. A None
#'   dimension means the feature column should be treat as variable length and
#'   will be parsed as a `SparseTensor`.
#' @param default_value A single value compatible with dtype or a list of values
...
#'   the default value for every dimension. If a list of values is provided, the
#'   length of the list should be equal to the value of `dimension`. Only scalar
#'   default value is supported in case dimension is not specified.
#' @param dtype defines the type of values. Default value is tf.float32. Must be
#'   a non-quantized, real integer or floating point type.
#' @param normalizer If not None, a function that can be used to normalize the
#'   value of the real valued column after default_value is applied for parsing.
#'   Normalizer function takes the input tensor as its argument, and returns the
#'   output tensor. (e.g. lambda x: (x - 3.0) / 4.2). Note that for variable
#'   length columns, the normalizer should expect an input_tensor of type
#'   `SparseTensor`.
#'   
#' @family feature_column wrappers
#'   
...
#' 
#' @param sparse_id_column A `_SparseColumn` which is created by for example
...
#' @param ckpt_to_load_from (Optional). String representing checkpoint
#'   name/pattern to restore the column weights. Required if
#'   `tensor_name_in_ckpt` is not None.
#' @param tensor_name_in_ckpt (Optional). Name of the `Tensor` in the provided
#'   checkpoint from which to restore the column weights. Required if
#'   `ckpt_to_load_from` is not None.
#' @param max_norm (Optional). If not None, embedding values are l2-normalized
#'   to the value of max_norm.
#' @param trainable (Optional). Should the embedding be trainable. Default is
#'   True.
#'   
#' @family feature_column wrappers
#'   
...
#' 
#' @param columns An iterable of _FeatureColumn. Items can be an instance of
#'   _SparseColumn, _CrossedColumn, or _BucketizedColumn.
#' @param hash_bucket_size An int that is > 1. The number of buckets.
#' @param combiner A string specifying how to reduce if there are multiple
#'   entries in a single row. Currently "mean", "sqrtn" and "sum" are supported,
#'   with "sum" the default. "sqrtn" often achieves good accuracy, in particular
...
#'   checkpoint from which to restore the column weights. Required if
#'   `ckpt_to_load_from` is not None.
#' @param hash_key Specify the hash_key that will be used by the
#'   `FingerprintCat64` function to combine the crosses fingerprints on
#'   SparseFeatureCrossOp (optional).
#'   
#' @family feature_column wrappers
#'   
#' @export
...
#' ``` 
#' 
#' This configuration assumes that input dictionary of model contains
#' the following two items: 
#' 
#' - (key="sparse_col", value=sparse_tensor) where sparse_tensor is
#'    a SparseTensor. 
#' - (key="weights_col", value=weights_tensor) where weights_tensor is
#'    a SparseTensor. 
#' 
...
#' 
#' @param sparse_id_column A `_SparseColumn` which is created by
#'   `column_sparse_with_*` functions.
#' @param weight_column_name A string defining a sparse column name which
#'   represents weight or value of the corresponding sparse id feature.
#' @param dtype Type of weights, such as `tf.float32`. Only floating and integer
#'   weights are supported. Returns: A _WeightedSparseColumn composed of two
#'   sparse features: one represents id, the other represents weight (value) of
#'   the id feature in that example.
#'   
#' @return A _WeightedSparseColumn composed of two sparse features: one
#'   represents id, the other represents weight (value) of the id feature in
#'   that example. Raises: ValueError: if dtype is not convertible to float.
#'   
...
#' 
#' @param sparse_id_column A _SparseColumn which is created by
#'   `column_sparse_with_*` or crossed_column functions. Note that `combiner`
#'   defined in `sparse_id_column` is ignored.
#'   
...
#' @param boundaries A list or list of floats specifying the boundaries. It has
#'   to be sorted.
#'   
...
#' @family feature_column wrappers
#'   
#' @section Raises: ValueError: if 'boundaries' is empty or not sorted.
#'   
#' @export
