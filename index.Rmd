...
Next we define the feature columns for our model. Feature columns are mappings of raw input data to the data that we'll actually feed into our training, evaluation, and prediction steps. To define feature columns we call the `feature_columns()` function then pipe column definitions into it. For example:

```{r}
fc <- feature_columns(mtcars,
  column_numeric(disp),
  column_numeric(cyl)
)
```
