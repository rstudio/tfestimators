---
title: "Creating Estimators - Abalone Age Predictor"
output: 
...
indicating the context in which the model_fn was invoked:
  *   `mode_keys()$TRAIN` The `model_fn` was invoked in training
mode, namely via a `train()` call.
*   `mode_keys()$EVAL`. The `model_fn` was invoked in
evaluation mode, namely via an `evaluate()` call.
*   `mode_keys()$PREDICT`. The `model_fn` was invoked in
