...
```{r eval=TRUE}
parsing_ops <- tf$python$ops$parsing_ops
expected_spec <- list(
  a = parsing_ops$FixedLenFeature(tuple(1L), dtype = tf$float32),
  c = parsing_ops$FixedLenFeature(tuple(1L), dtype = tf$float32),
  b = parsing_ops$FixedLenFeature(tuple(1L), dtype = tf$int64)
)
